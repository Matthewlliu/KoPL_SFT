...
from util import formatting_input_func
...
def parse_args():
    ...
    parser.add_argument("--dataset_text_field", type=str, default=None, help="the text field of the dataset")
    ...
    parser.add_argument("--train_size", type=str, default='1w')
...
class LoraTrainer:
    ...
    def load_dataset(self):
        train_file = os.path.join(self.dataset_path, 'train_1w_formated_2.json')
        eval_file = os.path.join(self.dataset_path, 'val_formated_2.json')
    ...
    def train(self):
        ...
        trainer = SFTTrainer(
            ...
            #formatting_func=formatting_input_func,
...
if __name__ == '__main__':
    ...
    time_label = time.strftime("%Y-%m-%d-%H:%M", time.localtime())
    model_name = args.model_name.split('/')[-1]
    save_folder_name = '_'.join([model_name, 'kopl', time_label, \
                "epoch%s"%args.num_train_epochs, "bs%s"%args.batch_size, "lr%s"%args.learning_rate, \
                args.train_size])
    args.output_dir = args.output_dir.format(save_folder_name)
    args.cache_dir = args.cache_dir.format(save_folder_name)
